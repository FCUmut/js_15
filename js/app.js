class CalorieTracker {
  ...
  removeMeal(id) {
    const index = this._meals.findIndex((meal) => meal.id === id); // will return '-1' if there is no match
    if (index !== -1) {
  ...
  removeWorkout(id) {
    const index = this._workouts.findIndex((workout) => workout.id === id); // will return '-1' if there is no match
    if (index !== -1) {
...
class Storage {
  static getCalorieLimit(defaultLimit = 2100) {
    let calorieLimit;
  ...
  static getMeals() {
    ...
    if (localStorage.getItem("meals") === null) {
      ...
      meals = JSON.parse(localStorage.getItem("meals")); // since meals are array not a number, we parse into array with JSON.parse
    }
    return meals;
  }
  ...
  static clearAll() {
    ...
    // If you want to include 'Daily Limit', this is shortcut
    // localStorage.clear();
  }
}
...
class App {
  ...
  _removeItem(type, e) {
    if (
      ...
      if (confirm("Are you sure?")) {
        const id = e.target.closest(".card").getAttribute("data-id"); // the closest div that has ".card" attribute
  ...
  _setLimit(e) {
    ...
    this._tracker.setLimit(+limit.value); // to send it as number we add '+' prefix
